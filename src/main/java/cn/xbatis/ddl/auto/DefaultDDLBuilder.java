...
public class DefaultDDLBuilder implements DDLBuilder {
    ...
    /**
     * 根据已解析的实体上下文生成单独修改自增策略的 SQL。
     */
    protected String buildModifyColumnAutoIncrementSql(DDLContext context, ColumnInfo column) {
        if (!supportsSeparatedModifyAutoIncrement(context.dbType)) {
            throw new UnsupportedOperationException(context.dbType.getName() + " does not support separated MODIFY AUTO_INCREMENT");
        }
        StringBuilder ddl = new StringBuilder();
        ddl.append("ALTER TABLE ");
        appendTableName(ddl, context);
        if (isDm(context.dbType)) {
            ...
            ddl.append(context.dbType.wrap(column.getName()));
            ddl.append(" DROP IDENTITY)");
        } else if (context.dbType == DbType.DB2) {
            ddl.append(" ALTER COLUMN ");
            ddl.append(context.dbType.wrap(column.getName()));
            ddl.append(" ");
            if (isAutoIncrement(column, context.idColumnCount)) {
                ddl.append("SET GENERATED BY DEFAULT AS IDENTITY");
            } else {
                ddl.append("DROP IDENTITY");
            }
        } else {
            ...
            ddl.append(context.dbType.wrap(column.getName()));
            ddl.append(" ");
            if (isGauss(context.dbType)) {
                if (isAutoIncrement(column, context.idColumnCount)) {
                    ddl.append("SET DEFAULT nextval('").append(buildSerialSequenceName(context, column)).append("')");
                } else {
                    ddl.append("DROP DEFAULT");
                }
            } else if (isAutoIncrement(column, context.idColumnCount)) {
                ddl.append("ADD GENERATED BY DEFAULT AS IDENTITY");
            } else {
                ddl.append("DROP IDENTITY IF EXISTS");
            }
        }
        ddl.append(";");
        return ddl.toString();
    ...
    protected String buildModifyColumnDefinitionSql(DDLContext context, ColumnInfo column) {
        ...
        return buildColumnSql(context, column, false, false, isAutoIncrement(column, context.idColumnCount)).trim();
    }
     ...
     */
    @Override
    public List<String> modifyColumnAutoIncrementSqlList(IDbType dbType, TableInfo tableInfo, Collection<ColumnInfo> columns, String tableName) {
        Objects.requireNonNull(dbType, "dbType");
        Objects.requireNonNull(tableInfo, "tableInfo");
        Objects.requireNonNull(columns, "columns");
        Objects.requireNonNull(tableName, "tableName");
        ...
        if (!supportsSeparatedModifyAutoIncrement(dbType)) {
            throw new UnsupportedOperationException(dbType.getName() + " does not support separated MODIFY AUTO_INCREMENT");
        }
        ...
        for (ColumnInfo column : context.columns) {
            boolean autoIncrement = isAutoIncrement(column, context.idColumnCount);
            if (isGauss(context.dbType) && autoIncrement) {
                sqlList.add(buildCreateSerialSequenceSql(context, column));
            }
            sqlList.add(buildModifyColumnAutoIncrementSql(context, column));
            if (isGauss(context.dbType) && !autoIncrement) {
                sqlList.add(buildDropSerialSequenceSql(context, column));
            }
        }
        return sqlList;
    }
    ...
    protected String buildCreateSerialSequenceSql(DDLContext context, ColumnInfo column) {
        ...
        ddl.append(buildSerialSequenceName(context, column));
        ddl.append(" OWNED BY ");
        appendTableName(ddl, context);
    ...
    /**
     * 生成 Gauss 去掉自增时清理残留序列的 DROP SEQUENCE SQL。
     */
    protected String buildDropSerialSequenceSql(DDLContext context, ColumnInfo column) {
        return "DROP SEQUENCE IF EXISTS " + buildSerialSequenceName(context, column) + ";";
    }

    /**
     * 生成与 PostgreSQL serial 约定一致的序列名：{table}_{column}_seq。
     */
    protected String buildSerialSequenceName(DDLContext context, ColumnInfo column) {
        String sequenceName = context.tableName + "_" + column.getName() + "_seq";
        if (sequenceName.length() > 63) {
            sequenceName = sequenceName.substring(0, 63);
        }
        if (context.tableInfo.getSchema() != null && !context.tableInfo.getSchema().isEmpty()) {
            sequenceName = context.dbType.wrap(context.tableInfo.getSchema()) + "." + sequenceName;
        }
        return sequenceName;
    }
    ...
    protected String buildColumnSql(DDLContext context, ColumnInfo column, boolean includePrimaryKey, boolean includeUnique) {
        return buildColumnSql(context, column, includePrimaryKey, includeUnique, includePrimaryKey);
    }

    /**
     * 生成单个字段的列定义 SQL。
     */
    protected String buildColumnSql(DDLContext context, ColumnInfo column, boolean includePrimaryKey, boolean includeUnique, boolean includeAutoIncrement) {
        return buildColumnSql(context.dbType, column, context.columns, context.idColumnCount, includePrimaryKey, includeUnique, includeAutoIncrement);
    }
    ...
    protected String buildColumnSql(IDbType dbType, ColumnInfo column, List<ColumnInfo> columns, boolean includePrimaryKey, boolean includeUnique) {
        return buildColumnSql(dbType, column, columns, getIdColumnCount(columns), includePrimaryKey, includeUnique, includePrimaryKey);
    }
    ...
    protected String buildColumnSql(IDbType dbType, ColumnInfo column, List<ColumnInfo> columns, int idColumnCount, boolean includePrimaryKey, boolean includeUnique) {
        return buildColumnSql(dbType, column, columns, idColumnCount, includePrimaryKey, includeUnique, includePrimaryKey);
    }

    /**
     * 生成单个字段的列定义 SQL。
     */
    protected String buildColumnSql(IDbType dbType, ColumnInfo column, List<ColumnInfo> columns, int idColumnCount,
                                    boolean includePrimaryKey, boolean includeUnique, boolean includeAutoIncrement) {
        StringBuilder ddl = new StringBuilder();
        ddl.append("  ").append(dbType.wrap(column.getName()));
        ...
        if (autoIncrement && includeAutoIncrement) {
    ...
    protected boolean isGauss(IDbType dbType) {
        return dialect.isGauss(dbType);
    }

    protected boolean isDm(IDbType dbType) {
        return dialect.isDm(dbType);
    }
    ...
    /**
     * 判断数据库是否支持在 MODIFY COLUMN 定义中内联修改自增策略。
     */
    protected boolean supportsInlineModifyAutoIncrement(IDbType dbType) {
        return dialect.supportsInlineModifyAutoIncrement(dbType);
    }

    /**
     * 判断数据库是否支持用独立 ALTER COLUMN 语句修改自增策略。
     */
    protected boolean supportsSeparatedModifyAutoIncrement(IDbType dbType) {
        return dialect.supportsSeparatedModifyAutoIncrement(dbType);
    }
