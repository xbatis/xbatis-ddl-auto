    ...
    void db2ShouldModifyPrimaryKeyAutoIncrementInSyncMode() throws Exception {
        try (Connection connection = openDatabaseConnectionOrSkip()) {
            DDLAutoExternalDatabaseIntegrationSupport.assertSyncModifyAutoIncrementFlow(
                    DbType.DB2,
                    connection,
                    "ALTER TABLE auto_sync_modify_auto_id_user ALTER COLUMN id SET GENERATED BY DEFAULT AS IDENTITY;"
            );
        }
    }

    @Test
    void db2ShouldRemovePrimaryKeyAutoIncrementInSyncMode() throws Exception {
        try (Connection connection = openDatabaseConnectionOrSkip()) {
            DDLAutoExternalDatabaseIntegrationSupport.assertSyncModifyAutoIncrementReverseFlow(
                    DbType.DB2,
                    connection,
                    "ALTER TABLE auto_sync_modify_auto_id_reverse_user ALTER COLUMN id DROP IDENTITY;"
            );
        }
    }
