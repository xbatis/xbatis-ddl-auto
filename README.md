...
`UPDATE` 模式只会自动新增字段和缺失索引；`SYNC` 模式会额外删除多余字段和索引、修改字段定义（类型、长度、默认值）和索引，但仍不会自动执行以下高风险操作：
...
`SYNC` 会修改字段类型、字段长度和默认值，不会修改字段可空性或重命名字段。
...
- 新增字段时，MySQL/MariaDB/OceanBase 会按实体字段顺序生成 `AFTER <列名>` 子句（多列新增合并为单条语句，每个新增列引用最近的前置已存在列，避免 `AFTER` 引用同批次新增列），使物理表列顺序与实体一致
